...
namespace RMXUtils
{
	...
	*/
	...
	/*!
	* Gets the command line string associated with the process.
	*/
	std::wstring GetProcessCommandLine(unsigned long process_id);

	/*!
	* Disable screen capture for the specific window
	* \param hWnd : A handle to the top-level window. The window must belong to the current process.
	* \param disable: True to disable. False to enable
	* \return: If successful. Note: If the window has already marked as anti-screencapture, skip but return true.
	*/
	BOOL DisableScreenCapture(HWND hWnd, bool disable);

#if !defined(_NOEXCEPT) && (_MSC_VER>1900)
#if _HAS_EXCEPTIONS
