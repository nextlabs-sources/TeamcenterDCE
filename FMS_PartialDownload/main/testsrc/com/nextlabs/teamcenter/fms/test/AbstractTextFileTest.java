 ...
 * 
 * @author clow
 * Test cases for plaintext text file and encrypted text file
 ...
 * 9. Assert random access case 9, fileoffset: 1024000, off: 0, len: 1024000
 * 10. Assert random access case 10 [zero byte file], fileoffset: 1024, 0ff: 0, len: 1024
 * 
 */

public abstract class AbstractTextFileTest {
	
	private File plaintextFile = new File(TXT_PLAINTEXTFILE_PATH);
	private File encryptedFile = new File(TXT_ENCRYPTEDFILE_PATH);
	
	private File plaintextEmptyFile = new File(TXT_PLAINTEXTEMPTYFILE_PATH);
	private File encryptedEmptyFile = new File(TXT_ENCRYPTEDEMPTYFILE_PATH);
	...
	public void assertRandomAccess1() throws Exception {
		...
		String plaintextData = new String(bufferPlaintext);
		...
		int encryptedByteRead = nxlDecryptingInputStream.read(
				fileOffset, bufferEncrypted, new InterfaceBufferHandler() {
					public void onBuffer(final ByteBuffer bbData, final int iBufOffset, final int iBufSize) throws Exception {
						...
						strBldrDecrypted.append(decryptedData);
		...
		String decryptedData = strBldrDecrypted.toString();
		...
		if (SHOWDATA) {
			...
			LOGGER.info("=============		DECRYPTED DATA		============={}{}",
					System.lineSeparator(), decryptedData.toString());
		}
		...
		// Assert
		assertEquals(plaintextData.toString(), decryptedData.toString());
	}
	...
	public void assertRandomAccess3() throws Exception {
		...
		String plaintextData1 = new String(bufferPlaintext1);
		String plaintextData2 = new String(bufferPlaintext2);
		...
		int encryptedByteRead1 = nxlDecryptingInputStream.read(
				fileOffset1, bufferEncrypted1, new InterfaceBufferHandler() {
					public void onBuffer(final ByteBuffer bbData, final int iBufOffset, final int iBufSize) throws Exception {
						...
						String decryptedData = new String(data);
						strBldrDecrypted1.append(decryptedData);
		...
		int encryptedByteRead2 = nxlDecryptingInputStream.read(
				fileOffset2, bufferEncrypted2, new InterfaceBufferHandler() {
					public void onBuffer(final ByteBuffer bbData, final int iBufOffset, final int iBufSize) throws Exception {
						...
						String decryptedData = new String(data);
						strBldrDecrypted2.append(decryptedData);
		...
		String decryptedData1 = strBldrDecrypted1.toString();
		String decryptedData2 = strBldrDecrypted2.toString();
		...
		if (SHOWDATA) {
			...
			LOGGER.info("=============		DECRYPTED DATA		============={}{}", 
					System.lineSeparator(), decryptedData1.toString());
		}
		...
		if (SHOWDATA) {
			...
			LOGGER.info("=============		DECRYPTED DATA		============={}{}", 
					System.lineSeparator(), decryptedData2.toString());
		}
		LOGGER.info("3. End => Assert random access 3");
		LOGGER.info("");
		LOGGER.info("");
		...
		assertEquals(plaintextData1.toString(), decryptedData1.toString());
		assertEquals(plaintextByteRead2, encryptedByteRead2);
		assertEquals(plaintextData2.toString(), decryptedData2.toString());
	}
	...
	public void assertRandomAccess8() throws Exception {
		...
		byte[] trimPlaintext = new byte[plaintextByteRead];
		System.arraycopy(bufferPlaintext, 0, trimPlaintext, 0, plaintextByteRead);
		
		String plaintextData = new String(trimPlaintext);
	 ...
	 * fileoffset: 148000, off: 0, len: 1024000
	 */
	@Test
	...
	/**
	 * assert random access case 10
	 * fileoffset: 1024, off: 0, len: 1024
	 */
	@Test
	public void assertRandomAccess10() throws Exception {
		// test case configuration
		int len = 1024;
		int bbSize = 64;
		int fileOffset = 1024;
		...
		long startPlaintext = System.nanoTime();
		FileInputStream fis = new FileInputStream(plaintextEmptyFile);
		BufferedInputStream bis = new BufferedInputStream(fis);
		byte[] bufferPlaintext = new byte[len + offset];
		...
		if (plaintextByteRead > 0) {
			byte[] trimPlaintext = new byte[plaintextByteRead];
			System.arraycopy(bufferPlaintext, 0, trimPlaintext, 0, plaintextByteRead);
			
			plaintextData = new String(trimPlaintext);
		} else {
			plaintextData = "";
		}
		...
		long startEncrypted = System.nanoTime();
		TestFMSFileReaderStream fisProtected = new TestFMSFileReaderStream(
				new FileInputStream(encryptedEmptyFile));
		AbstractNextLabsDecryptingInputStream nxlDecryptingInputStream = 
				getNextLabsDecryptingInputStream(fisProtected);
		nxlDecryptingInputStream.init();
		...
		// Test result
		LOGGER.info("10. Start [{}] => Assert random access 10 (fileoffset:{}, off:{},len:{})", 
				this.getClass().getSimpleName(), fileOffset, offset, len);
		LOGGER.info("	plaintext byteread: {}", plaintextByteRead);
		LOGGER.info("	encrypted byteread: {}", encryptedByteRead);
		LOGGER.info("	plaintext read(us): {}", (endPlaintext - startPlaintext) / 1000);
		...
		LOGGER.info("10. End => Assert random access 10");
		LOGGER.info("");
		LOGGER.info("");
