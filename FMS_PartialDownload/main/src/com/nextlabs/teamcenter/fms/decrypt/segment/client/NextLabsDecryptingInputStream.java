...
public class NextLabsDecryptingInputStream extends AbstractNextLabsDecryptingInputStream {
	...
	private static FMSRMXInstance rightsManager;
	...
	static {
		try {
			...
			rightsManager = new FMSRMXInstance();
		...
		if (rightsManager != null && fileHeader != null) {
			if (nxlState == null) {
				try {
					...
					long[] outputData = new long[2];
					
					if (rightsManager.buildNxlHeader(
								Arrays.copyOf(fileHeader, fileHeader.length), outputData)) {
						long plaintextFileSize = outputData[0];
						nxlState = new NxlFile(fileHeader, plaintextFileSize);
					} else {
						LOGGER.info("nxlState is set to null due to buildNxlHeader returns false");
					}
			...
			if (nxlState != null) {
				try {
					tempPlaintextFileLength = nxlState.getContentLength();
				} catch (Exception ex) {
					LOGGER.error(ex.getMessage(), ex);
	...
	public int read(final long fileOffset, final ByteBuffer bb,
		...
		if (isEncrypted()) {
			...
			InterfaceBufferHandler nxlBufferHandler = new NextLabsBufferHandlerClient(
