...
sub printUsage
{
	...
	print "  namepostfix   Specify the postfix of artifact file name: bin|install|build.\n";
...
sub parseCommandLine()
{
	...
	if (!GetOptions(
			...
			'namepostfix:s' => \$g_fileNamePostfix, # --namepostfix
	...
	if ($g_fileNamePostfix eq '')
	{
		# This is optional 
		print "File name postfix not specified. build.zip or bin.zip will be picked.\n";
		return 1;
	}
...
sub getProductBuildArtifactFile()
{
	...
	#			print "Matched = $1\n";
		...
		{
			if ($entry =~ /(.*-$g_fileNamePostfix\.zip)$/)
			{
	#			print "Matched = $1\n";
...
print "  g_fileNamePostfix = $g_fileNamePostfix\n";
