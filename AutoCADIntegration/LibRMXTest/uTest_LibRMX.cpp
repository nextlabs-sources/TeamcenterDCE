...
*/
		...
		{
			Logger::WriteMessage("TestProtectFile:");
			wstring filePath = g_testDataDir + FILE_UNPROTECT;
			wstring destPath = g_testDataDir + L"TestProtectFile\\" + FILE_PROJECTED;
		...
		{
			Logger::WriteMessage("TestIsProjectedFile:");
		...
		{
			Logger::WriteMessage("TestAddRPMFolder:");
			bool ret;
		...
		{
			Logger::WriteMessage("TestProtectFileInRPMFolder:");
			fs::path folderPath(g_testDataDir + L"TestProtectFileInRPMFolder");
			fs::create_directories(folderPath);
			fs::copy(g_testDataDir, folderPath.wstring());

			bool ret;

			ret = RMX_AddRPMDir(const_cast<wchar_t*>(folderPath.c_str()));
			Assert::IsTrue(ret, L"AddRPMDir not working");
			...
			ret = RMX_ProtectFile(filePath.c_str(), nullptr, destPath.c_str());
			Assert::IsTrue(ret, L"Protection file failed");
			Assert::IsTrue(RMX_IsProtectedFile(destPath.c_str()), L"No protect file is generated");
		}
		...
		{
			Logger::WriteMessage("TestCopyNxlFileFromRPM:");
			bool ret;
		...
		{
			Logger::WriteMessage("TestCopyNxlFileToRPM:");
		...
		{
			Logger::WriteMessage("TestCopyNxlFileRPMToRPM:");
			bool ret;
		...
		{
			Logger::WriteMessage("TestRemoveRPMFolder:");
		...
		{
			Logger::WriteMessage(L"TestGetTagsFromRPM");
		...
		{
			Logger::WriteMessage(L"TestCheckRightsInRPM");
		...
		{
			Logger::WriteMessage("TestRMXLastError:");
		...
		{
			Logger::WriteMessage("TestGetActivitiyInfo:");
