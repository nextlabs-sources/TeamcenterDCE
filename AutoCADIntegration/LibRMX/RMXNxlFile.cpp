...
bool CRMXNxlFile::CheckRights(unsigned long rights, bool audit)
{
	...
	auto callCheck = [&](DWORD dwRight) -> bool {
		bool grantedRight = false;
		if (dwRight & lFileRights)
		{
			grantedRight = true;
		}

		const wstring& rightNamesToCheck = GetRightName(dwRight);
		LOG_INFO_FMT("CheckRights('%s'): %s, Result: %d ", m_nxlFilePath.c_str(), rightNamesToCheck.c_str(), grantedRight);

		if (audit)
			AddActivityLog(sR2LogMap[dwRight], grantedRight ? RMX_RAllowed : RMX_RDenied, false);

		return 	grantedRight;
	};
	// Extract multiple rights to check
	if (HAS_BIT(rights, RMX_RIGHT_VIEW) && !callCheck(RMX_RIGHT_VIEW))
		return false;

	if (HAS_BIT(rights, RMX_RIGHT_EDIT) && !callCheck(RMX_RIGHT_EDIT))
		return false;

	if (HAS_BIT(rights, RMX_RIGHT_PRINT) && !callCheck(RMX_RIGHT_PRINT))
		return false;

	if (HAS_BIT(rights, RMX_RIGHT_DOWNLOAD) && !callCheck(RMX_RIGHT_DOWNLOAD))
		return false;

	if (HAS_BIT(rights, RMX_RIGHT_DECRYPT) && !callCheck(RMX_RIGHT_DECRYPT))
		return false;

	if (HAS_BIT(rights, RMX_RIGHT_SAVEAS) && !callCheck(RMX_RIGHT_SAVEAS))
		return false;

	if (HAS_BIT(rights, RMX_RIGHT_CLIPBOARD) && !callCheck(RMX_RIGHT_CLIPBOARD))
		return false;

	if (HAS_BIT(rights, RMX_RIGHT_SCREENCAPTURE) && !callCheck(RMX_RIGHT_SCREENCAPTURE))
		return false;

	if (HAS_BIT(rights, RMX_RIGHT_CLASSIFY) && !callCheck(RMX_RIGHT_CLASSIFY))
		return false;

	if (HAS_BIT(rights, RMX_RIGHT_SHARE) && !callCheck(RMX_RIGHT_SHARE))
		return false;
