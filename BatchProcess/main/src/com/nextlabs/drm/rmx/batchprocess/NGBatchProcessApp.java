...
public class NGBatchProcessApp {
...
private final static Logger LOGGER;
	...
	public static void main(String[] args) {
		NGBatchProcessApp batchProcessApp = new NGBatchProcessApp();
		...
		if (action == null || inputFilePath == null) {
			...
			LOGGER.info("SkyDRMBatchProcess started with the following parameters: ");
			LOGGER.info("action=" + action + ", inputFilePath=" + inputFilePath);
			LOGGER.info("");
			
			NXLPropertiesFile profileProps = null;
			try {
				profileProps = new NXLPropertiesFile(NXLPropertiesFile.SKYDRMPROFILE_PROPERTIES_FILE);
			} catch (Exception ex) {
				LOGGER.error("Error: failed to load skydrmprofile.properties");
				System.exit(1);
			}
			...
			ConfigPropertiesDTO configProp = new ConfigPropertiesDTO(
					profileProps.getRouterURL(), profileProps.getTenantName(), profileProps.getAppID(), profileProps.getAppKey());
			...
			switch (action) {
				...
				case PROTECT:
					try {
						batchProcess = new BatchEncrypt(tc_user_name, stagingFoldPath, numThreads, session.getConnection(), configProp);
	...
	private void printUsage() {
		System.out.println("");
		System.out.println("Usage: skydrmbatchprocess -a <action> -f <input>");
		System.out.println("");
		System.out.println("Options:");
		System.out.println("\t-a\t\taction: protect.");
