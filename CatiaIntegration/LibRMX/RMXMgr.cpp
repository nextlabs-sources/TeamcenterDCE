...
DWORD ReadFile(const std::wstring& file, std::vector<UCHAR>& blob)
{
...
DWORD WriteNxlFile(HANDLE fFile, const std::vector<UCHAR>& blob)
{
	if (blob.size() == 0)
		return ERROR_INVALID_PARAMETER;

	unsigned __int64 contentSize = blob.size();
	const unsigned char* content = blob.data();
	...
	if (!WriteFile(fFile, buf.data(), (ULONG)buf.size(), &bytesWritten, NULL)) {
		return GetLastError();
	}
...
bool CRMXMgr::ProtectFile(const string& tags, const wstring& filePath, const wstring& newNxlFilePath)
{
	...
	if (newNxlFile.CopyTo(newNxlFilePath, true))
	{
		...
		LOG_INFO_FMT(L"ProtectFile completed (tags: %s): '%s'", RMXUtils::s2ws(normalizedTags).c_str(), destNxlFile.GetNxlFilePath().c_str());
		return true;
	}
	return false;
}
...
void CRMXMgr::NotifyMessage(const std::wstring& filePath, const std::wstring& msg)
{
	CRMXLastResult result;
	
	static wstring processName;
	if (processName.empty())
	{
		wchar_t szProcessPath[MAX_PATH];
		if (GetModuleFileName(NULL, &szProcessPath[0], MAX_PATH) == 0)
		{
			result = CSysErrorMessage(GetLastError());
			CHK_ERROR(true, L"Cannot get the file name of current process (error: %s)", result.ToString().c_str());
			return;
		}

		CPathEx processPath(szProcessPath);
		processName  = processPath.GetFileNameWithExt();
	}

	auto pDRmcInstance = CRMXInstance::GetInstance().GetDRmcInstance();
	CHK_ERROR(pDRmcInstance == nullptr, L"Failed to NotifyMessage: '%s' (error: Invalid pDRmcInstance)", filePath.c_str());

	result = pDRmcInstance->RPMNotifyMessage(processName, filePath, msg, 1);
	CHK_ERROR(!result, L"[RPM]NotifyMessage failed (error: %s)", result.ToString().c_str());
