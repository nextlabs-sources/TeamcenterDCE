...
bool CRMXInstance::Initialize()
{
	...
	do
	{
		dwWaitTime++;
		bFinished = false;
		result = m_pDRmcInstance->IsInitFinished(bFinished);
		LOG_INFO_FMT(L"[RPM]IsInitFinished: %d(time:%d, error: %s)\r\n", bFinished, dwWaitTime, result.ToString().c_str());
	...
	if (!bFinished)
	{
		result = CRMXResult(100001, L"Timeout to wait PDP connection. Please retry to start later.");
		CHK_ERROR_RETURN(!result, false, L"[RPM]Cannot initialize RMX. IsInitFinished failed (error: %s)", result.ToString().c_str());
	}
