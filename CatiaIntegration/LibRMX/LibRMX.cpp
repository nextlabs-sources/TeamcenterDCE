...
LIBRMXAPI bool RMX_RPMSetViewOverlay(const RMX_VIEW_OVERLAY_PARAMS& params)
{
	...
	{
		CRMXNxlFile nxlFile(params.files.fileArr[i]);
		vecFiles.push_back(nxlFile.GetNxlFilePath());
	}
...
LIBRMXAPI bool RMX_HasRights(const wchar_t* nxlFilePath, unsigned long allRights, unsigned long rightsToCheck, bool audit)
{
	if ((nxlFilePath == nullptr) || (allRights == 0))
		return false;

	CRMXNxlFile nxlFile(nxlFilePath);
	return nxlFile.HasRights(allRights, rightsToCheck, audit);
}

LIBRMXAPI void RMX_NotifyMessage(const wchar_t* nxlFilePath, const wchar_t* msg)
{
	CRMXMgr::GetInstance().NotifyMessage(nxlFilePath, msg);
}