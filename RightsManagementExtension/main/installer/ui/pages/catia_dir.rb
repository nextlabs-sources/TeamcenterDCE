...
# Pages         :: CATIA Installation Directory
# Description   :: Render CATIA installation directory page and register into Shoes.url()
...
# GUI CATIA Install Directory page
class Installation < Shoes
    include Utility
    url('/catia_dir', :catia_dir)
        ...
        stack @installer_page_size_style do
            ...
            stack @installer_content_size_style do
                ...
                stack width: 1.0, height: 80 do
                    para MESSAGE['catia_dir_page']['heading'], @large_text_style.merge(stroke: '#000000', align: 'center', margin_top: 20, margin_bottom: 20, margin_right: 20)
                end
                ...
                stack width: 760, height: 345, margin_left: 200 do
                    flow width: 1.0, height: 120 do
                        para MESSAGE['catia_dir_page']['catia_dir'], @small_text_style

                        stack width: 430, height: 50, margin_top: 10 do
                            @catia_dir_edit = nl_text_edit(:large, @item.catia_dir, width: 430, height: 40)
                            @catia_dir_edit.change do
                                # use "/" to be consistent
                                @item.catia_dir = @catia_dir_edit.text.tr('\\', '/')
                            end
                        end
                        ...
                        stack width: 100, height: 50, margin_top: 10 do
                            button MESSAGE['browse_folder_btn'], width: 80, height: 40 do
                                ...
                                @catia_dir_edit.text = browse_result unless browse_result.nil?
                            end
                        end
                        para MESSAGE['catia_dir_page']['help_note'], @small_text_style.merge(size: 8)
                    end
                end
            ...
            stack @installer_footer_size_style do
                ...
                flow width: 1.0, height: 60 do
                    stack width: 180, height: 50 do
                        catia_dir_back_btn = nl_button :back
                        catia_dir_back_btn.click do
                            visit(wizard(:back))
                        end
                    end
                    ...
                    stack width: 180, height: 50 do
                        catia_dir_next_btn = nl_button :next
                        catia_dir_next_btn.click do
                            @item.catia_dir = @catia_dir_edit.text.strip

                            # Verify catia_dir
                            validated, error_msg = @item.validate_input(:catia_dir, @item.catia_dir)
                            if !validated
                                alert_ontop_parent(app.win, error_msg, title: app.instance_variable_get('@title'))
                                @catia_dir_edit.focus
                            else
                                visit(wizard(:next))
                            end
                        end
