...
# Recipes       :: upgrade_partialdownload
...
ruby_block 'upgrade_partialdownload' do
    block do
        log_info(LOGMSG::UPGRADE_PARTIALDOWNLOAD)
        # in case of accidental break
        set_break_point(node['temp_dir'], recipe_name)

        log_info('Stop FCC process')
        fcc_stop = File.join(node['TC_ROOT'], 'tccs', 'bin', 'fccstat')
        fcc_stop = fcc_stop.tr('/', '\\') if RUBY_PLATFORM =~ /mswin|mingw|windows/
        fcc_stop_cmd = "\"#{fcc_stop}\" -stop | exit 0"
        call_system_command('Stop FCC process', 'fccstat', fcc_stop_cmd)

        # Basically uninstall old PartialDownload solution
        log_info('Remove old RMX PartialDownload feature')
        ...
        # Redeploy RMX 5.2 Partial Download solution
        nxl_dir = File.join(node['TC_ROOT'], 'tccs', 'third_party', 'nxl')
        ...
        if check_vis_server_installed(node['TC_ROOT'])
            ...
            FileUtils.copy_entry(src_dir, nxl_dir)
            # copy_files(src_dir, nxl_dir)
