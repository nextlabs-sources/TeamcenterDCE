...
# Recipes       :: install_nxl_template
# Description   :: Deploy nextlabs template, import tc preferences
...
ruby_block 'deploy_nextlabs_template' do
    # Template will be "nextlabs", "nextlabsipem" and "nextlabsswim"
    ...
    block do
        log_info(LOGMSG::INSTALL_NXL_TEMPLATE)
        ...
        if node['feature_list'].include?('nxl_template')
            ...
            elsif node['tc_version'] == '12'
                nxl_data_model = File.join(ENV['RMX_ROOT'], 'DataModel', 'nextlabs_tc12')
            else
                nxl_data_model = File.join(ENV['RMX_ROOT'], 'DataModel', 'nextlabs_tc13')
            end
        ...
        if node['feature_list'].include?('nxl_ipem_template')
            ...
            if node['tc_version'] == '10'
                ...
                nxl_ipem_data_model = File.join(ENV['RMX_ROOT'], 'DataModel', 'nextlabs_ipem', 'tc13')
            end
        ...
        if node['feature_list'].include?('nxl_swim_template')
            ...
            if node['tc_version'] == '11'
                ...
                nxl_swim_data_model = File.join(ENV['RMX_ROOT'], 'DataModel', 'nextlabs_swim', 'tc13')
            end
...
ruby_block 'import_tc_preferences' do
    block do
        # Only when install nextlabs template (+ customize foundation) then we should import preferences. To avoid override existing preferences
        if node['feature_list'].include?('nxl_template')
