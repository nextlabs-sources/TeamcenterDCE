...
public class NextLabsUnprotectCmdIsEnabled extends AbstractSourceProvider {
	
	public final static String NXL_UNPROTECT_CMD_IS_ENABLED = "com.nextlabs.menuitem.cmd.unprotect.isEnabled";
	...
	public String[] getProvidedSourceNames() {
		return new String[] {NXL_UNPROTECT_CMD_IS_ENABLED};
	}
	...
	public Map<String, String> getCurrentState() {
		...
		currentState.put(NXL_UNPROTECT_CMD_IS_ENABLED, currentStateVal);
	...
	private void getNxlPreference() {
		...
		try {
			...
			if ((localAbstractAIFSession instanceof TCSession) && 
				...
				isEnabled = tcPrefService.getLogicalValue(NXL_UNPROTECT_ENABLE);
				setNxlUnprotectCmdIsEnabled(isEnabled);
			}
		} catch (Exception ex) {
	...
	public void setNxlUnprotectCmdIsEnabled(boolean enable) {
		if (isEnabled == enable)
			return; // no change
		
		isEnabled = enable;
		String currentStateVal = isEnabled ? TRUE : FALSE;
		fireSourceChanged(ISources.WORKBENCH, NXL_UNPROTECT_CMD_IS_ENABLED, currentStateVal);
	}
