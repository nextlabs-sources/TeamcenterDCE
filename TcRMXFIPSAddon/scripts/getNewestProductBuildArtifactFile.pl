...
#		- 26PS_perf
#		- 81PS_officepep
#	3. Build artifact zip file always ends with -bin.zip or -build.zip or -setup.zip
#		For example:
...
#       - doom-SDK-10.4.1470-release-install.zip
#
...
sub printUsage
{
	...
	print "  namesubstring   Specify the substring include inside artifact file name: CreoRMX|NXRMX|...\n";
	print "  buildnumber   Specify the build to pick. Optional. If it's not specified, newest one will be picked.\n";
}
...
sub parseCommandLine()
{
	...
	if (!GetOptions(
			...
			'namesubstring:s' => \$g_fileNameSubString, # --namesubstring
			'buildNumber:i' => \$g_buildNumber	# --buildNumber
		))
	{
		return 0;
	}
	...
	if ($g_fileNameSubString eq '')
	{
		# This is optional 
		print "File name substring not specified. build.zip or bin.zip will be picked.\n";
		return 1;
	}
	...
	{
		# This is optional 
		print "Build number not specified. Newest build will be picked.\n";
		return 1;
	}
...
sub getProductBuildArtifactFile()
{
	...
	#			print "Matched = $1\n";
		...
		{
			if ($entry =~ /(.*$g_fileNameSubString.*\.zip)$/)
			{
	#			print "Matched = $1\n";
...
print "  g_buildNumber     = $g_buildNumber\n";
print "  g_fileNameSubString = $g_fileNameSubString\n";
